...
def plot_srt(
    ...
    *,
    ...
    line_p, = ax_left.plot(x, p, lw=1.2, label="Pressure vs √t")
    ax_left.set_xlabel("√t [s$^{0.5}$]")
    ax_left.set_ylabel("Pressure [bar]")
    ax_left.grid(True, ls="--", lw=0.7, alpha=grid_alpha)

    # Right axis: dP/d√t vs √t
    ax_right = ax_left.twinx()
    line_d, = ax_right.plot(x, d, lw=1.2, ls="--", label="dP/d√t")
    ax_right.set_ylabel("dP/d√t [bar s$^{-0.5}$]")
...
def plot_gfunction(
    ...
    *,
    ...
    G-function diagnostic with two y-axes:
      ...
      - Right y-axis: Semilog derivative of P vs G
    ...
    # Right axis: semilog derivative vs G
    ax_right = ax_left.twinx()
    line_d, = ax_right.plot(G, semilog_dP, lw=1.2, ls="--", color="tab:red", label="Semilog derivative")
    ax_right.set_ylabel("Semilog derivative (G·dP/dG) [bar]", color="tab:red")
    ax_right.tick_params(axis="y", labelcolor="tab:red")

    # Optional x-limit using time cap converted to G cap
    if cap_s is not None and np.isfinite(cap_s):
        # find G at the capped time
        t_cap = min(float(cap_s), float(np.nanmax(t)) if len(t) else float(cap_s))
        # last index where t <= t_cap
