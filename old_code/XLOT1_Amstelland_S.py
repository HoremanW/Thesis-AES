...
# ----------------------------
# √t diagnostic (pressure + normal & semilog derivatives) — SURFACE
# ----------------------------
x_sqrt, p_srt, dpdx = closure_analysis.derivative_vs_sqrt_time(ts_surf, p_surf, max_t_s=cap_s)
...
ax_srt_left.set_title("√t diagnostic (SURFACE pressure & derivatives)")

if i_srt is not None:
    p_closure_srt = p_srt[i_srt]
    print(f"[SURFACE] FCP (√t min) : {p_closure_srt:.2f} bar at t ≈ {x_sqrt[i_srt]**2:.0f} s")

# ----------------------------
# Bourdet diagnostic (pressure + dP/dln t) — SURFACE
# ----------------------------
t_log, dP_dlogt = closure_analysis.bourdet_derivative(ts_surf, p_surf, smooth_win=None, max_t_s=cap_s)
...
figBRD, axBRD = plotting.plot_bourdet(
    t_log, dP_dlogt,
    p=p_surf,        # plot SURFACE pressure on left axis
    p_times=ts_surf, # interpolate onto t_log if needed
    i_cl=i_brd,
    cap_s=cap_s
)
axBRD.set_title("Bourdet diagnostic (SURFACE pressure & dP/dln t)")

if i_brd is not None:
    print(f"[SURFACE] FCP (Bourdet): {p_surf[i_brd]:.2f} bar at t ≈ {t_log[i_brd]:.0f} s")

# ----------------------------
# G-function diagnostic (pressure + semilog & normal derivatives) — SURFACE
# ----------------------------
...
axG.set_title(f"G-function diagnostic (SURFACE, t_p ≈ {tp_s:.0f} s)")

# ----------------------------
# Intersection methods (√t & G) — SURFACE
# ----------------------------
res_srt = closure_analysis.fcp_by_sqrt_intersection(
    ts_surf, p_surf, max_t_s=cap_s, min_left=8, min_right=8
)
if res_srt.get('ok'):
    print(f"[SURFACE] FCP (√t intersection): {res_srt['closure_pressure_bar']:.2f} bar "
          f"at t ≈ {res_srt['closure_time_s']:.0f} s")

res_g = closure_analysis.fcp_by_g_intersection(
    ts_surf, p_surf, tp_seconds=tp_s, max_t_s=cap_s, min_left=8, min_right=8
)
if res_g.get('ok'):
    print(f"[SURFACE] FCP (G intersection) : {res_g['closure_pressure_bar']:.2f} bar "
          f"at t ≈ {res_g['closure_time_s']:.0f} s")

# Plots for the intersection fits — SURFACE
fig_srt_int, ax_srt_int = plotting.plot_srt_intersection(
    ts_seconds=ts_surf, p=p_surf, res_srt=res_srt, cap_s=cap_s
)
ax_srt_int.set_title("√t intersection (SURFACE)")

fig_g_int, ax_g_int = plotting.plot_g_intersection(
    ts_seconds=ts_surf, p=p_surf, res_g=res_g, tp_seconds=tp_s, cap_s=cap_s
)
ax_g_int.set_title(f"G-function intersection (SURFACE, t_p ≈ {tp_s:.0f} s)")
...
# ----------------------------
# PER-CYCLE diagnostics & plots (SURFACE basis for every detected cycle)
# ----------------------------
# Tip: remove the duplicate detect_pump_cycles() call above to avoid confusion.

MIN_T_S_FOR_PICK = 0        # e.g., set to 90 to ignore the first 90 s
CAP_PER = 180.0             # per-cycle analysis window cap (s), surface clock

if len(cycles) == 0:
    print("No cycles detected — nothing to analyze.")
else:
    print("\nPer-cycle closure analysis on SURFACE pressure (Bourdet & G-function within 0–180 s):")
    for i, cyc in enumerate(cycles, 1):
        # --- Cycle window on SURFACE clock (no lag shift) ---
        ...
        # --- Bourdet derivative pick (global minimum) on SURFACE ---
        ...
        # --- G-function diagnostic (Barree/Nolte) on SURFACE ---
        # Per-cycle pumping time on SURFACE clock
        try:
        ...
        # ===========================
        # √t INTERSECTION METHOD + overlay (SURFACE)
        # ===========================
        try:
            res_srt = closure_analysis.fcp_by_sqrt_intersection(
        ...
        if res_srt.get("ok"):
            ...
            # x-range split at closure (√t domain)
        ...
        figBRD, axBRD = plotting.plot_bourdet(
            t_log, dP_dlogt,
            p=p_s,           # SURFACE pressure samples
            p_times=ts_s,    # their time base (sec since shut-in)
            i_cl=i_brd,
            cap_s=cap_local
        )
        if i_brd is not None:
            axBRD.set_title(f"Cycle {i} — Bourdet min at ~{t_log[i_brd]:.0f}s, P≈{p_s[i_brd]:.2f} bar (SURFACE)")
        else:
            axBRD.set_title(f"Cycle {i} — no Bourdet min in [{int(MIN_T_S_FOR_PICK)}, {int(cap_local)}] s (SURFACE)")
        ...
        if i_g is not None:
            axG.set_title(
                f"Cycle {i} — G-function (SURFACE, tp={tp_s:.0f}s), "
                f"min semilog dP at t≈{ts_s[i_g]:.0f}s, P≈{p_s[i_g]:.2f} bar"
            )
        else:
            axG.set_title(
                f"Cycle {i} — G-function (SURFACE, tp={tp_s:.0f}s), "
                f"no min in [{int(MIN_T_S_FOR_PICK)}, {int(cap_local)}] s"
            )

        # =======================
        # G INTERSECTION METHOD + overlay (SURFACE)
        # =======================
        try:
            res_g = closure_analysis.fcp_by_g_intersection(
        ...
        # -----------------
        # Print the results (SURFACE)
        # -----------------
        if i_brd is not None:
            print(f"  Cycle {i}: FCP (Bourdet) ≈ {p_s[i_brd]:.2f} bar at t ≈ {t_log[i_brd]:.0f} s "
                  f"(ended by {cyc['ended_by']}, window {cap_local:.0f}s)")
        else:
            print(f"  Cycle {i}: no Bourdet minimum found in [{MIN_T_S_FOR_PICK}, {cap_local}] s (SURFACE).")

        if i_srt is not None:
            print(f"            FCP (√t)      ≈ {p_srt[i_srt]:.2f} bar at t ≈ {x_sqrt[i_srt]**2:.0f} s")

        if i_g is not None:
            print(f"            FCP (G-func)  ≈ {p_s[i_g]:.2f} bar at t ≈ {ts_s[i_g]:.0f} s (tp={tp_s:.0f}s)")
