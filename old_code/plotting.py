...
from . import closure_analysis as ca
...
def plot_surface_panels(
    ...
    density_array=None,
    volume_array=None,
    tot_volume_nrd_array=None,
    return_volume_array=None,
    totbalance_volume_array=None,
    RDA_pressure_array=None,
    *,
    figsize=(22, 8),
    markersize=0.2,
    linewidth=1.0,
    ...
    """
    Flexible surface panels plotter.

    - Only plots the series that are provided (non-None) and have the same length as time_array.
    - Returns (fig, axs), where axs is an array of matplotlib Axes.
    - Backward-compatible: can still take all arrays as before.
    """

    # Ensure time array exists
    if time_array is None:
        raise ValueError("time_array is required")

    n = len(time_array)

    # Ordered list of all possible series
    series = [
        (pressure_array,          "Treating Pressure [bar]"),
        (flowrate_array,          "Flowrate [m³/h]"),
        (density_array,           "Density [kg/m³]"),
        (volume_array,            "Pumped Volume [m³]"),
        (tot_volume_nrd_array,    "TOT Volume NRD [m³]"),
        (return_volume_array,     "Return Volume [m³]"),
        (totbalance_volume_array, "Total Balance [m³]"),
        (RDA_pressure_array,      "RDA Pressure [bar]"),
    ]
    ...
    if not panels:
        raise ValueError("No valid input series provided (must match length of time_array).")

    # Create subplots (vertical stack)
    fig, axs = plt.subplots(len(panels), 1, sharex=True, figsize=figsize)
    if not isinstance(axs, (list, np.ndarray)):
        axs = np.array([axs])

    # Plot each valid series
    for ax, (y, ylabel) in zip(axs, panels):
        ax.plot(time_array, y, marker='o', markersize=markersize,
                linestyle='-', linewidth=linewidth)
        ax.set_ylabel(ylabel, fontsize=title_labelsize)
    ...
    axs[-1].set_xlabel("Time", fontsize=title_labelsize)
...
def plot_gfunction(
    ...
    *,
    ...
    cap_s=None,
    figsize=(10, 5),
    plot_normal=False  # optional: show orange normal derivative
):
    """
    G-function diagnostic:
      - Left y-axis : Pressure vs G(ts)
      - Right y-axis: Semilog derivative (G·dP/dG) [always], normal derivative [optional]
    """
    ...
    # Align pressure to time grid if needed
    if p_times is not None and len(p_arr) != len(t):
        pt = np.asarray(p_times, float)
        m = np.isfinite(pt) & np.isfinite(p_arr)
        if m.any():
    ...
    # Right axis: semilog derivative (always), normal derivative (optional)
    ax_right = ax_left.twinx()
    line_semilog, = ax_right.plot(
        G, semilog_dP, lw=1.2, ls="--", color="tab:red",
        label="Semilog derivative (G·dP/dG)"
    )
    lines = [line_p, line_semilog]
    labels = [line_p.get_label(), line_semilog.get_label()]

    if plot_normal and dP_dG is not None:
        line_normal, = ax_right.plot(
            G, dP_dG, lw=1.2, ls="-.", color="tab:orange",
            label="Normal derivative (dP/dG)"
        )
        lines.append(line_normal)
        labels.append(line_normal.get_label())

    ax_right.set_ylabel("Derivative(s) [bar]")
    ...
    # Ensure semilog line "starts at top-left"
    finite = np.isfinite(semilog_dP)
    if finite.any():
        i0 = int(np.where(finite)[0][0])
        y0 = float(semilog_dP[i0])
        ymin = float(np.nanmin(semilog_dP[finite]))
        ymax = float(np.nanmax(semilog_dP[finite]))
        # Put the first point close to the top, and include full range
        pad = 0.1 * max(1e-6, (ymax - ymin))
        # Keep higher values at the TOP: upper limit near the first value
        ax_right.set_ylim(y0 + pad, ymin - pad)
...
# --- plotting.py additions ---

import numpy as np
import matplotlib.pyplot as plt

def _apply_layout(fig, top=0.86, pad=1.8, hspace=0.45, wspace=0.25):
    """Loosen layout so long titles/legends don't clip in widget view."""
    try:
...
def plot_barree_tangent(
    G, P, y_semilog, res_barree, cycle_idx=None,
    title_pad=10, label_P="Pressure fall-off", label_y=r"$-G\,\mathrm{d}P/\mathrm{d}G$",
    autoscale_deriv=True, deriv_margin_frac=0.10
):
    """Always plot P vs G (left) and semilog derivative (right). Overlay tangent/marker if pick ok."""
    def _safe_layout(fig, top=0.86, pad=1.8, hspace=0.45, wspace=0.25):
        try:
            fig.tight_layout(pad=pad, rect=[0, 0, 1, top])
            fig.subplots_adjust(top=top, hspace=hspace, wspace=wspace)
        except Exception:
    ...
    fig, axP = plt.subplots(1, 1, figsize=(6.6, 4.2))
    axD = axP.twinx()

    # Left: pressure (always)
    axP.plot(G, P, lw=1.4, color="tab:blue", label=label_P)
    axP.set_xlabel("G-function")
    ...
    # Right: semilog derivative (always)
    axD.plot(G, y_semilog, lw=1.2, color="tab:red", label=label_y)
    axD.set_ylabel(label_y, color="tab:red")
    axD.tick_params(axis="y", labelcolor="tab:red")
    axD.grid(False)

    # Autoscale derivative axis
    if np.isfinite(y_semilog).any():
        y_min, y_max = np.nanmin(y_semilog), np.nanmax(y_semilog)
        if np.isfinite(y_min) and np.isfinite(y_max) and y_max != y_min:
            margin = deriv_margin_frac * (y_max - y_min)
            axD.set_ylim(y_min - margin, y_max + margin)

    title = "Barree tangent"
    if res_barree.get("ok"):
        m = float(res_barree.get("m"))
        line = m * np.asarray(G, float)
        Gc = float(res_barree.get("Gc"))
        Pc = float(res_barree.get("Pc"))
        yGc = float(np.interp(Gc, G, y_semilog))

        axD.plot(G, line, lw=1.0, ls="--", color="k", label=rf"Tangent $y=mG$ (m={m:.3g})")
        axD.axvline(Gc, color="gray", lw=0.9, ls=":", label=r"$G_c$")
        axD.scatter([Gc], [yGc], s=22, color="k", zorder=4)
        axP.scatter([Gc], [Pc], s=25, color="tab:blue", edgecolor="k", zorder=5)

        title += " (origin line touches semilog)"
    else:
        title += " (no valid pick — semilog shown)"

    if cycle_idx is not None:
        title = f"Cycle {cycle_idx} — {title}"
    axP.set_title(title, pad=title_pad)

    # Combined legend
    lP, nP = axP.get_legend_handles_labels()
    lD, nD = axD.get_legend_handles_labels()
    axP.legend(lP + lD, nP + nD, loc="best", fontsize=8)

    _safe_layout(fig)
    return fig, (axP, axD)


def plot_haimson_dpdt_vs_p(
    P, dPdt, res_haimson, cycle_idx=None, title_pad=10,
    label_pts=r"data: $\mathrm{d}P/\mathrm{d}t$ vs $P$"
):
    """dP/dt vs P with two linear fits and intersection if available."""
    def _safe_layout(fig, top=0.86, pad=1.8, hspace=0.45, wspace=0.25):
        try:
            fig.tight_layout(pad=pad, rect=[0, 0, 1, top])
            fig.subplots_adjust(top=top, hspace=hspace, wspace=wspace)
    ...
    title = "Haimson bilinear: dP/dt vs P"

    if res_haimson.get("ok"):
        Pc = float(res_haimson["Pstar"])
        aL, bL = float(res_haimson["slopeL"]), float(res_haimson["interceptL"])
        aR, bR = float(res_haimson["slopeR"]), float(res_haimson["interceptR"])

        ax.scatter(P, dPdt, s=10, alpha=0.65, label=label_pts)
        if np.isfinite(P).any():
            Pline = np.linspace(np.nanmin(P), np.nanmax(P), 100)
            ax.plot(Pline, aL*Pline + bL, lw=1.2, label="Left fit")
            ax.plot(Pline, aR*Pline + bR, lw=1.2, label="Right fit")
        ax.axvline(Pc, color="k", lw=0.9, ls=":", label=r"$P^*$ (intersection)")
    else:
        title += " (not available)"
    ...
    ax.set_ylabel(r"$\mathrm{d}P/\mathrm{d}t$ (bar/s)")
...
def plot_haimson_dpdt_vs_p(
    ...
    """
    Haimson bilinear plot: dP/dt vs P with two linear fits and intersection.
    Inputs:
      P, dPdt: arrays (cleaned)
      res_haimson: dict (expects ok, Pstar, slopeL, interceptL, slopeR, interceptR)
    Returns: fig, ax
    """
    fig, ax = plt.subplots(1, 1, figsize=(6.6, 4.2))
    ...
    if res_haimson.get("ok"):
        ...
        title = "Haimson bilinear: not available"
        if cycle_idx is not None:
            title = f"Cycle {cycle_idx} — " + title
        ax.set_title(title, pad=title_pad)
        ax.set_xlabel("Pressure P (bar)")
