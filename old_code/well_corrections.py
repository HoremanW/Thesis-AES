...
def pressure_drop_darcy(Q, D, L, rho, mu = 0.5 * 10**(-3), eps=0.0, K_sum=0.0, g=9.80665):
    """Return Δp_total [Pa] for incompressible flow using Darcy–Weisbach + minor losses."""
    # mu is assumed to be 0.5 * 10-3, which is the dynamic viscosity for water at 50 degrees C
    A = math.pi * (D/2)**2
    v = Q / A
    Re = rho * v * D / mu
    if Re < 2300 and Re > 0:
        ...
        # Swamee–Jain explicit turbulent correlation
        f = 0.25 / (math.log10(eps/(3.7*D) + 5.74/(Re**0.9))**2)
    dp_fric  = f * (L/D) * 0.5 * rho * v**2
    dp_minor = K_sum * 0.5 * rho * v**2
    return dp_fric + dp_minor, dict(v=v, Re=Re, f=f, dp_fric=dp_fric, dp_minor=dp_minor)
